Function AuditGroupMembership
	...
	$group = Read-Host -Prompt "Group to look up"

	# Replace all illegal characters with "." so the account name can be used in a filename
	[System.IO.Path]::GetInvalidFileNameChars() | % {$accountvalid = $group.replace($_,'.')}

	$time = Get-Date -format M-d-yyyy
	$groupfile = "$home\Documents\$accountvalid-Nested-Group-Membership.txt"
	$Cmdlet = Get-NestedMember

	echo "$time" > "$groupfile"
	echo "------------" >> "$groupfile"
	echo "$group" >> "$groupfile"
	echo "`n" >> "$groupfile"

	if ( "$Cmdlet" -ne "Get-NestedMember" )
...
#>
	...
	echo "###################################### Nested Groups ######################################" >> "$accountfile"

echo $(Get-NestedMember "$group" | Group Name | Select name, count | Sort-Object count -Descending) >> "$groupfile"

echo "`n" >> "$groupfile"
	}
			

Read-Host "Script complete! See $groupfile"
			}
